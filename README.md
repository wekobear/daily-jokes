...
这是年度编号约定，不按标准 SemVer 的破坏性变更划分第一位；兼容性变化另作说明。依赖版本、API／数据结构版本不随项目改号。历史发布自 `v0.1.0` 起按年度方案连续编号，下载地址与标签对应；旧验证记录及本地旧包保留用于追溯，不代表现行版本。
...
[多路研究与筛选实例](docs/research-v0.5.0.md)包含 11 个保留方向、来源核验边界及未通过的候选，不直接将所有模型输出当作已审核笑话。
...
安装包仅包含明确清单中的通用文件，不包含私有运行区或配置。v0.4.0 新增通用视频服务流程、新手教程、当日选稿、Archify 流程展示和完整案例资源。v0.3.0 保留供回溯。
