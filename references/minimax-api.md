...
低成本联调可复用一张已有分镜，只提交一个 4 秒 H3 768P 镜头，按此费率输出估算为 ¥2；这只是单镜头试片，不是十五秒完整故事。H3-Max 480P 的十五秒输出估算为 ¥4.95，但本项目尚未实测该模型的画质和声音。合并镜头请求不降低按秒单价；更换模型、缩短时长不能悄悄改变用户要求的成片范围。
